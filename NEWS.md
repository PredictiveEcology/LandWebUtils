...
* new `landweb_sppEquiv()`, promoted from `LandWeb_preamble`'s `InitSpecies()`: adds the `LandWeb` column to `LandR::sppEquivalencies_CA` and relabels the groups that merge species. It is tested identical to the module's inline code on LandR's real table, kept as a fixture so `LandR` stays out of the package's dependencies, with one deliberate change: **`Abie_spp` is now labelled "Fir"**. It had no group label, so each of its rows kept its own species' name, and `Biomass_core`'s leading-vegetation maps, which read a group's label from its first row, called the whole group (subalpine fir, western redcedar and western hemlock included) "Balsam fir". Unlike the module, it copies its input rather than modifying the lazy-loaded `LandR` table by reference.
...
* new `cc_age_composite()`, promoted from `LandWeb_preamble`: crops a current-condition age raster to the study area, clears its NoData sentinel, and fills the cells it leaves empty from a second source aged forward to the same epoch. The fill never overwrites the primary source (a cell-wise minimum would drag inventory ages younger). Sentinels are cleared by value with `terra::classify()` rather than `NAflag<-`, because for an in-memory raster `NAflag<-` converts the values but `NAflag()` still reports `NaN`, so the module's assertion on the flag would fail on correct data.
* new `cc_age_pct_missing()`, promoted from `LandWeb_preamble`'s `ccAgeMaxMissing` gate: the percent of forest in the study area with no age. Each of the three wrong denominators it replaced is pinned by a test -- the raster's bounding box (WesternAlbertaUpland read 71.4% missing against a true 3.9%), the whole study area (lakes counted as missing age), and a coarser grid (which loosened the figure 3-20x). The forest mask uses `terra::classify()` rather than `ifel(x %in% classes)`: `%in%` only reaches terra's `SpatRaster` method when terra is attached, and resolves to base `%in%` inside a package namespace.
* new `lcc2020_classes()`: the single definition of which Land Cover of Canada 2020 codes LandWeb treats as forest, non-flammable, urban, wetland, cropland and shrub/grass/moss, previously repeated as literals across `LandWeb_preamble`.
* new `lcc2020_remap_table()`, promoted verbatim from `LandWeb_preamble`: the `overlayLCCs()` remap table for combining the simulation land cover with LCC 2020, preserving v2's rule order. Equivalence with the module's inline code is tested over 200 randomised class sets. Its first rule (`LCC` 0/20/30 to `NA`) is overwritten for every code in the LCC 2020 legend and only applies to codes outside it; that is documented and pinned rather than changed.
...
* a related defect is deliberately **not** fixed and is pinned by a test: the pattern-based fuel typing can disagree with `landmine_known_species()`. `Lari_spp` matches none of `Pice`/`Pinu`/`Popu` so it is typed `softwood`, while `knownSpecies` calls larch `decid`; they disagree, the join finds no match, and larch also falls through to `ROSother`. The mixedwood fix does not sweep it up, because there the pattern-derived value would override the authoritative mapping. Larch is 3 pixels on WesternAlbertaUpland;
* `youngFilter` now defaults to `"always"`, applying the young class's species filter unconditionally as its two siblings already do. This makes **no difference on a landscape containing pine** (0 of 4,458,168 WesternAlbertaUpland pixels differ) but prevents a pine-free study area from silently assigning every young stand `ROSother` instead of its `immature_young` rate. `"legacy"` remains available to reproduce older runs;
